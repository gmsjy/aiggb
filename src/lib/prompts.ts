...
function buildPromptBase(appMode: "2d" | "3d", domain: Domain, phase: "full" | "compile" = "full"): string {
  ...
  return phaseIntro + "\n\n" + [
    ...
    "2. 颜色 #RRGGBB。SetColor r/g/b 用 0~1 浮点（引擎按 ×255 解析；0~255 整数会渲染成白色）。",
...
10. ★ 属性命令（SetColor / SetLineOpacity / SetFilling / style op）专项修复：
   - SetColor 收到大于 1 的整数（如 (230, 50, 50)）→ 各分量除以 255 重发（→ (0.902, 0.196, 0.196)）。
     r/g/b 是 0~1 浮点（引擎按 ×255 解析），与透明度同值域；0~255 整数会被钳成白色。
   ...
   - 「Sequence/Zip 里嵌 SetVisibleInView / SetColor 执行失败」→ Set*/Show* 是**语句**不是表达式，
     不能嵌进 Sequence/Zip/If；批量显隐/样式逐条输出命令（一条一个 eval），
     或 eval_raw 内用换行符分隔多条命令（实测可行；分号分隔无效）。

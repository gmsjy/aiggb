...
export const TEMPLATES: Template[] = [
  ...
  },
  {
    ...
    prompt: "单摆（小角近似）。注入 g=9.8。参数：L=1(0.1~2 step0.05 m 摆长)，theta0=π/6(0~π/4 step0.01 rad 初角)，t=0(0~10 step0.02 s)。omega=sqrt(g/(L+0.001))，theta=theta0*cos(omega*t)。O=(0,0)，M=(L*sin(theta), -L*cos(theta)) 红。Segment(O,M) 蓝。轨迹 M 蓝拖尾。eval Text(\"小角近似：θ₀≤45°，周期误差<4%\", (0.7,0.1)) 深灰。动画 t oscillating。",
    domain: "physics", mode: "2d"
  },
  {
    id: "field-dipole", icon: "🧿", title: "偶极子电场", subtitle: "正负电荷电场线对称弧族",
    prompt: "等量异种电荷电场。参数：d=2(1~3 step0.1)，q=3(1~5 step1)。A=(d,0) 红，B=(-d,0) 蓝。上半电场线 Sequence(CircumcircleArc(A,(0,j),B), j, 0.4, 2.6, 0.45) 红，下半 Sequence(CircumcircleArc(A,(0,-j),B), j, 0.4, 2.6, 0.45) 红。中垂线 Segment(B,A) 红虚线。方向箭头 Vector((-0.5,0),(0.2,0)) 黑。标注 +q @A、−q @B。轴 x/m y/m。",
    domain: "physics", mode: "2d"
  },
  ...
  // ═══════ 2D · 数学（4 个） ═══════
  {
    ...
    prompt: "正弦函数族 y=A*sin(kw*x+phi)。参数：A=1(0.5~3 step0.1 振幅)，kw=1(0.5~5 step0.1 频率)，phi=0(-π~π step0.05 rad 相位)。f(x)=A*sin(kw*x+phi) 蓝。y=A、y=-A 灰虚线（振幅边界）。轴 x y。",
    domain: "general", mode: "2d"
  },
  {
    id: "unit-circle", icon: "⭕", title: "单位圆与三角函数", subtitle: "角 θ 对应 sin/cos 线段",
    prompt: "单位圆与三角函数。参数：theta=0(0~2π step0.01 rad)。O=(0,0)，Circle(O,1) 蓝。P=(cos(theta),sin(theta)) 红。Segment(O,P) 绿。垂线 Segment(P,(cos(theta),0)) 橙（sin 值）、Segment(P,(0,sin(theta))) 紫（cos 值）。动画 theta 自动。",
    domain: "general", mode: "2d"
  },
  {
    id: "conic", icon: "🥚", title: "圆锥曲线", subtitle: "椭圆 · 双曲线 · 抛物线对比",
    prompt: "圆锥曲线对比。参数：a=3(0.5~4 step0.1 半长轴)，b=2(0.5~4 step0.1 半短轴)，p=3(0.5~4 step0.1 抛物线焦距)。椭圆 x^2/a^2+y^2/b^2=1 蓝。双曲线 x^2/a^2-y^2/b^2=1 绿虚线。抛物线 y=x^2/(4p) 紫虚线（独立参数 p，不借用椭圆 a）。ce=sqrt(abs(a^2-b^2)+0.001)（椭圆焦距）。椭圆焦点 E1=If(a>=b, (ce,0), (0,ce)) E2=If(a>=b, (-ce,0), (0,-ce)) 红。ch=sqrt(a^2+b^2+0.001)（双曲线焦距，注意是加号）。双曲线焦点 H1=(ch,0) H2=(-ch,0) 绿。抛物线焦点 Fp=(0,p) 紫。轴 x y。",
    domain: "general", mode: "2d"
  },
  {
    id: "cycloid", icon: "🔁", title: "摆线", subtitle: "圆沿直线滚动 · 圆周点轨迹",
    prompt: "摆线（圆在 x 轴上滚动，圆周点轨迹）。参数：r=1(0.5~3 step0.05)，t=0(0~4π step0.02)。圆心 C=(r*t, r)，圆 Circle(C,r) 浅蓝。P=(r*t-r*sin(t), r-r*cos(t)) 红。Segment(C,P) 绿。轨迹 P 蓝。x 轴灰虚线。动画 t 自动。",
    domain: "general", mode: "2d"
  },
  ...
  // ═══════ 3D 立体（4 个，3D 模式下全量展示） ═══════
  {
    ...
    prompt: "3D 正方体截面。顶点全显式声明：A=(0,0,0) B=(3,0,0) C=(3,3,0) D=(0,3,0)（底面），E=(0,0,3) F=(3,0,3) G=(3,3,3) H=(0,3,3)（顶面，E 在 A 正上方）。cube=Prism(Polygon(A,B,C,D), E) 半透明蓝（显式顶点+Prism 构造，避免 Cube(A,B) 自动顶点命名与 C/F 显式声明冲突）。截面=IntersectPath(Plane(A,C,F), cube) 红（三角形 ACF）。A/C/F 红。3D 禁 SetViewDirection/SetFilling/SetPointSize/SetCaption/ZoomIn。",
    domain: "general", mode: "3d"
  },
  {
    id: "tetrahedron", icon: "🔺", title: "正四面体", subtitle: "等边三角锥 · 体积标注",
    prompt: "3D 正四面体。A=(0,0,0) B=(3,0,0) C=(1.5,2.598,0) D=(1.5,0.866,2.449)。tet=Tetrahedron(A,B,C,D) 半透明蓝。底面 ABC 橙轮廓。vol=Volume(tet) Text。Sphere(D,0.12) 红标记顶点。3D 禁 SetViewDirection/SetFilling/SetPointSize/SetCaption。",
    domain: "general", mode: "3d"
  },
  {
    id: "sphere-section", icon: "🔵", title: "球体截面", subtitle: "平面截球 · 截面圆高亮",
    prompt: "3D 球体截面。sph=Sphere((0,0,0),3) 半透明蓝。plane=Plane((0,0,1.5),Vector((0,0,1))) 半透明灰。截面=IntersectPath(plane,sph) 橙。球心/截面圆心点。虚线 Segment((0,0,0),(0,0,1.5)) 灰。3D 禁 SetViewDirection/SetFilling/SetPointSize/SetCaption。",
    domain: "general", mode: "3d"
  },
  {
    id: "helix", icon: "🧬", title: "螺旋运动", subtitle: "带电粒子在磁场中",
    prompt: "3D 螺旋线（带电粒子在匀强磁场）。参数：t=0(0~6π step0.02)。曲线 Curve(cos(t),sin(t),t/3,t,0,6π) 橙。动点 P=(cos(t),sin(t),t/3)，Sphere(P,0.12) 红。速度矢量：vTmp=Vector((0,0,0),(-sin(t),cos(t),1/3))，vArrow=Translate(vTmp,P) 绿（两步法防 Point+Point）。磁场 B 沿 z 轴紫：bTmp=Vector((0,0,0),(0,0,1))，B=Translate(bTmp,(0,0,0))。采样轨迹 Sequence(Sphere((cos(i*0.1),sin(i*0.1),i*0.1/3),0.05), i, 0, 59, 1) 淡紫。动画 t 自动。3D 禁 SetViewDirection/SetFilling/SetPointSize/SetCaption。",
    domain: "general", mode: "3d"
  },
];

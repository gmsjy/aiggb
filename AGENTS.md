...
| `ggbKB.ts` | **RAG 命令知识库** | `GGB_COMMAND_DEFS`（~130 条命令：签名/参数/2D3D 适用）、`HALLUCINATION_MAP`（30 条臆造→正确映射，含 El→Element / Round→round / SetVisible→SetVisibleInView）、`buildCommandReference(mode, domain)`、`buildHallucinationWarnings(mode)`、`findCommand`、`findHallucination` |
| `commandCorrect.ts` | 后置命令纠正器 | `correctCommand(cmd)`（Levenshtein ≤2 模糊纠正 + 臆造查表 + 参数个数校验）、`batchCorrect()`、`correctionsToRepairContext()` |
| `commandValidate.ts` | **执行前静态语法预检**（纯 TS） | `validateGGBCommand(cmd, mode?)` → `{ok, issues[], message}`：括号配对（含坐标括号与嵌套调用）、`).` 句点误用（逗号手误）、Sequence 参数契约（官方 5 种重载分支判定 / 循环变量单字母 / 区间压缩 / 尾部残留）、参数个数（ARG_COUNT_HINTS + 属性命令回退 ggbKB paramCount）、运算符结尾、**属性命令专项**（3D 模式禁令 `mode-forbidden` / SetColor 值域与色名 `value-range`/`color-name` / 透明度 0~1，mode 经 ggbBridge/toolExecutor 透传）、**保留对象名**（`reserved-name`：If/xAxis/yAxis/zAxis/x/y/z/e 赋值拦截，2026-09 整机实测新增）；`validateSequenceArgs(args)` → Sequence 契约/循环变量参数级校验（供 eval_raw 路径与 KB 自查复用）；被 `ggbBridge.executeOne`（eval）与 `toolExecutor`（eval_raw）统一调用，把无语义的引擎 false 变成可自愈的具体诊断 |
...
5. **语法预检层**：`commandValidate.ts`（**执行前**纯文本静态检查：括号配对、逗号误写成句点、Sequence 参数契约、参数个数、运算符结尾、**属性命令专项**——3D 模式禁令 / SetColor 值域 0~1 浮点与色名形态 / 透明度 0~1）——把引擎那句无语义的「执行失败」换成「具体错在哪 + 正确形态」，供修复回路精准自愈；`eval`/`eval_raw` 两条入口统一生效
...
- **evalCommand 不认科学计数法（5.4.927 实测）**：`c = 3e8` 被解析成 3×e×8（欧拉数）——带 e 的数值一律经 `physics.ts formatGGBNumber` 改写为幂形式（`3 * 10^(8)`）；注意 JS `String()` 仅在 |v|≥1e21 或 <1e-6 才输出 e 计数，`3e8`→"300000000" 无恙，而 Grav/eps0/h/k_B/e 五个常量旧代码长期被静默注入错值（如 Grav=6.6743×e−11≈7.14）。另：`getValueString` 按画布舍入位数显示（小值显示 "0"、独立数值带 `name = ` 前缀），程序化取值用 `getValueString(...).split('=').pop()` 或读 XML `<value val>`
- **3D 模式：`Cube(A,B)` 两点式优先；`SetViewDirection/SetFilling/SetPointSize/SetCaption` 等 3D 禁用**
- **Conic 六系数顺序（2026-09 二轮整机实测，与官方手册标注不符！）**：`Conic(a, b, c, d, e, f)` 在本 bundle (5.4.927) 实际解析为 **a·x² + b·y² + c·<常数> + d·xy + e·x + f·y = 0**——常数项在第 **3** 位、xy 在第 4 位（手册标注 `(ax²,by²,cxy,dx,ey,f)` 顺序会画出平移/旋转的错误圆锥曲线：实测 `Conic(1/9,1/4,0,0,0,-1)` → 中心 (0,2) 而非 (0,0)）。椭圆 x²/9+y²/4=1 的正确形态是 `Conic(1/9, 1/4, -1, 0, 0, 0)`（实测 0.44x²+y²=4 中心原点）。**7 个系数引擎 evalCommand 返回 true 却产出 emptyset**（空型对象、get_object_info 可见 "= ?"）——是"看似成功实则失败"的空转型陷阱，曾致 Agent 单轮 30 次迭代空转；预检 ARG_COUNT_HINTS `Conic: [5,6]` 已拦截（ggbKB 示例已同步实测顺序）
- **赋值形态双等号必失败**：`name = 表达式 = 值`（如 `ell = x^2/9 + y^2/4 = 1`）引擎直接报「创建函数/表达式 失败」——GGB 命名等式/圆锥曲线必须用**冒号**（`ell: x^2/9+y^2/4=1`）。commandValidate 第 ⑩ 项 `double-equals` 已静态拦截（字符串字面量内/括号内/`==` 比较符不误伤）
- **会话恢复必须回填画布模式**：`initSessionFromStorage` 恢复 `ggbAppName`（此前只恢复 messages/domain/agentMode——3D 会话刷新后模式回落 classic，快照经 setBase64 把 3D 透视带进 classic applet，store 与画布真实模式不一致，再点模式切换即清空画布）。快照消费三方（GGBCanvas appletOnLoad / store 即时恢复 / switchToSession）均按 `pendingCanvasSnapshotMode` 门控：applet 模式与快照目标模式不一致时不得 setBase64
- **E2E 诊断句柄**：主应用在 `appletOnLoad` 暴露 `window.__aiggbApi`（GGBCanvas.tsx）——deployggb 的 `window.ggbApplet` 在 applet 重建后指向陈旧实例（getObjectNumber 恒 0），自动化测试统一用 `__aiggbApi` 读取画布真相
...
- **属性命令（Set*/Show*/style op）**：`SetColor` r/g/b 是 **0~1 浮点**（引擎按 ×255 解析，官方手册一致；5.4.927 实测：`(0.9,0.2,0.2)`→rgb(229,51,51) 正确，`(230,50,50)`→rgb(255,255,255) 全白——旧约定「必须 0~255 整数、0~1 是误用」方向恰好相反，2026-09 实测修正，历史记录「0.9→230」恰是 0~1×255 的正确换算）；透明度/填充率同样是 **0~1 小数**（`SetLineOpacity`/`SetFilling`）；颜色名只能英文且需引号（中文色名/裸标识符必失败）。3D 禁令清单与替代方案见 `commandValidate.ts` 的 `MODE_3D_FORBIDDEN`（与 prompts MODE_3D_ADDON、ggbKB modes 三方一致——改任何一方须同步其余两方 + 测试）。完整报错模式分析见 `docs/属性命令报错分析.md`
- **对象显隐命令（5.4.927 实测）**：`SetVisible(obj, bool)` **不存在**（键表无此条目，evalCommand 静默 no-op 且恒返回 false——生产 create_spring/attach_vector 曾长期误用导致辅助对象从未真正隐藏，已改 `SetVisibleInView`）；显隐唯一命令形态是 `SetVisibleInView(obj, 1, true/false)`（截图实证隐藏生效；注意 `api.getVisible` 只读主标志位，视图掩码记在 XML `<show ... ev="...">`，get_object_info 已做两路合成）；条件显隐 `SetConditionToShowObject(obj, condition)` 可用（XML 写入 `<condition>`）；**查询显隐状态无命令形态**（`VisibleInView`/`GetVisibleInView`/`IsVisibleInView` 均不存在）——用 `get_object_info` 工具读取。`SetVisible`/`GetVisibleInView`/`IsVisibleInView` 已入 `HALLUCINATION_MAP`。**Set\* 是语句不是表达式**：嵌进 Sequence/Zip/If 引擎必拒（`Sequence(SetVisibleInView(Element({…},u),1,false),u,1,24)` 实测失败），批量显隐逐条发命令或 eval_raw 换行分隔（实测可行；分号分隔无效）——commandValidate 第 ⑦ 项 `scripting-nest` 已静态拦截
...
- **保留对象名与 f→If 误纠正（2026-09 整机 GUI 实测根因）**：对象赋值名禁用 GGB 保留名——`If` 是内置条件命令（实测 `If(x)=A*sin(kw*x+phi)` 创建的函数无法被 style 引用，样式修复多轮不收敛）、`xAxis`/`yAxis`/`zAxis` 是画布自带坐标轴**不需要也不可创建**（`xAxis=Line((0,0),(1,0))` 恒返回 false）、`x`/`y`/`z` 是坐标变量、`e` 是欧拉数（赋值后 `3e8` 被拆成 3×e×8）——commandValidate 第 ⑨ 项 `reserved-name` 已静态拦截。★ 连环根因：`commandCorrect` 旧版 `extractCommandName` 把函数定义 `f(x)=…` 的 `f` 当命令名，模糊匹配编辑距离 1 **误纠正成 `If(x)=…`**（AI 本来写对了！）——已修复为函数定义形态直接放行不参与纠正（回归测试在 `tests/ggbKB.test.ts`）。满足度评估曾误报「未检测到 x/y 轴对象」驱动修复 AI 去创建 xAxis——EVAL_SYSTEM_PROMPT 已加坐标轴豁免条款（画布自带轴，视觉审查 prompt 早有同款）
...
- **可注入确认钩子（E2E）**：Toolbar「清空」「切 2D↔3D」的确认走 `appConfirm()`（Toolbar.tsx）——默认原生 confirm，测试可注入 `window.__aiggbConfirm = () => true` 全自动通过（原生 confirm 同步阻塞渲染主线程，自动化协议无法应答且嵌入式 webview 对话框不可探测）
- **会话落盘时机**：`persistCurrentSession`（ChatPanel runRound finally，轮结束）+ `pagehide`/`visibilitychange→hidden` 兜底（ChatPanel effect）——后者保证轮间手动滑块/画布微调在刷新后不丢（实测 A=2 落盘→重载→保持；此前轮结束定格旧快照，重载即回退）
- **非流式 AI 调用兜底超时**：`AI_CALL_TIMEOUT_MS=180s`（aiClient.ts `withCallTimeout`，callAPI/ping 生效；流式 agentChat SSE 不适用）——防止挂起连接把评估修复链无限拖住；配合评估修复轮内层 `MAX_REPAIR` 收紧为 1（`executeAndRepair` 第 5 参），控制一轮最坏 ≈19 次 → 更低
